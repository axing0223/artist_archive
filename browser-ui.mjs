...
try{
 ...
 /* 两套视图共用一套说法：「作品数量：最新数量（截至日期数量）」，不再有独立的「数据截至日前作品」那一项。 */
 for(const view of ['comfortable','compact']){
  assert.match(density[view].countText,/^作品数量：\d+（\d+）$/,'两套视图的数量都要写成「作品数量：N（M）」：'+view+JSON.stringify(density[view]));
  assert.equal(density[view].inlineDisplay,'inline','截至量的括号两套视图都要显示：'+view+JSON.stringify(density[view]));
  assert.match(density[view].inlineText,/^（\d+）$/,'括号里要是截至日期之前的数量：'+view+JSON.stringify(density[view]));
 }
...
async function browserChecks(){
 ...
 check(specialButtons().map(node=>node.textContent).join('/')==='作品少于 50/测试风格图不是 2 张','特殊筛选的选项文案：'+specialButtons().map(node=>node.textContent).join('/'));
