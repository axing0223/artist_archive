...
try{
 ...
 assert.ok(density.compact.metaTop>=0&&density.compact.metaTop<=2,'紧凑视图的分类要贴在卡片顶部、不再破窗：'+JSON.stringify(density));
 assert.ok(density.compact.metaTop+density.compact.metaH<=density.compact.actionsTop,'顶部的分类横带不能压到右上角按钮：'+JSON.stringify(density));
