...
test('筛选支持反选：排除指定分类、标签、评分或特殊条件，并能与正选同时生效',()=>{
 const index=create(),works=n=>Array.from({length:n},(_,i)=>({id:String(i+1)}));
 const rows=[artist('a',{category:'场景',tags:['柔光'],score:4,works:works(5)}),artist('b',{category:'人物',tags:['柔光','自然'],score:5,works:works(5)}),artist('c',{category:'人物',tags:['自然'],score:null,works:works(2)})];
 assert.deepEqual(names(index.select(rows,{notTags:new Set(['柔光'])})),['c'],'反选标签 = 排除带这个标签的画师');
 assert.deepEqual(names(index.select(rows,{notCategory:'人物'})),['a'],'反选分类 = 排除这一类');
 assert.deepEqual(names(index.select(rows,{notScores:new Set([0])})),['a','b'],'反选评分 = 排除这些评分（未评分按 0 算）');
 assert.deepEqual(names(index.select(rows,{notSpecial:new Set(['no-works'])})),['a','b'],'反选特殊条件 = 排除符合它的画师');
 assert.deepEqual(names(index.select(rows,{tags:new Set(['自然']),notScores:new Set([5])})),['c'],'正选与反选可以同时生效');
 assert.deepEqual(names(index.select(rows,{notTags:new Set(['不存在'])})),['a','b','c'],'排除一个没人用的标签等于不筛');
});
