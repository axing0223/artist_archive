(() => {
  ...
  const namesForLookup=artist=>[artist.name,artist.alias,...(artist.aliases||[]),...(artist.tags||[])].filter(Boolean).map(normalizeForLookup);
  async function lookupForPrompt(tag){
    ...
    /* 只做「命中项包含查询」这一个方向：页面上双击只能选中 himura，库里存 himura kiseki，
       所以 himura 要能带上 kiseki。反方向（查询包含命中项）不要——标签里有 'd' 这种单字符，
       那个方向会把什么都捞进来。 */
