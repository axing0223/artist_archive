(() => {
  ...
  function render(){
    ...
    if(signature!==filterSignature){
      ...
      $('categories').replaceChildren(...['全部',...data.categories,'待判断'].map(c=>{
        /* 「全部」表达的是"没有按分类筛选"这个总状态，不是"名字叫全部的那一类被选中"。
           以前这里用 c===state.notCategory 判断，而 state.notCategory 存的是**被排除的那个分类名**
  ...
  /* 危险操作的统一交互：第一次点击只是「上膛」（换文案 + 变红，4 秒不动自动复原），
     再点一次才真的执行。node 用来接管界面上已经存在的按钮——导入备份那个按钮写在
     index.html 里、id 还被快捷操作列表引用，不能换成新节点，但交互要和删除画师完全一致。 */
  function confirmButton(label,armedLabel,onConfirm,cls='danger',node){
    let armed=false,timer=null,button=node;
    const fire=async()=>{
      if(busy||uploading)return;
      if(!armed){
  ...
  function bindExtensionMessages(){
    ...
    try{
      /* 待办领走后要回一条 ack，后台才会把它从队列里删掉。
         以前后台在交出来的那一刻就删了：页面随后被关掉/崩掉，这批建卡动作就永久消失，
         而用户已经看到「已记下这次添加」。现在没 ack 的下次打开画师库还会再来一遍
         （重复的建卡会被「已经在库里」挡住，所以重放是安全的）。 */
      const ack=ids=>{
        const requestIds=(ids||[]).filter(id=>typeof id==='string'&&id);
        if(!requestIds.length)return;
        try{runtime.sendMessage({channel:'artist-library-page',type:'ack',requestIds}).catch(()=>{});}catch{}
      };
      runtime.sendMessage({channel:'artist-library-page',type:'ready'}).then(answer=>{
        const actions=answer?.actions||[];
        for(const action of actions)handleAction(action,send);
        ack(actions.map(action=>action?.requestId));
      }).catch(()=>{});
    }catch{}
  }
