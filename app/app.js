(() => {
  ...
  let data,folder,draft,editingId,busy=false,uploading=false,volatile=false,viewerRef=null,refreshTimer=null;
  ...
  function editingCard(a){
    ...
    const tagsInput=el('input');tagsInput.value=draft.tags.join('、');tagsInput.placeholder='用顿号或逗号分隔，例如：厚涂、黑白';tagsInput.oninput=()=>draft.tags=unique(tagsInput.value.split(/[、,，;；\s]+/).filter(tag=>/[\p{L}\p{N}]/u.test(tag)));
  ...
  function listCategories(){
    $('category-list').replaceChildren(...data.categories.map(c=>{
      ...
      }),confirmButton('删除','确认删除？',async()=>{
        const used=data.artists.filter(a=>a.category===c).length;
  ...
  async function init(){
    ...
    $('viewer').addEventListener('close',()=>{ArtistImages.dispose('viewer');viewerRef=null;});$('save-original').onclick=saveViewerOriginal;
