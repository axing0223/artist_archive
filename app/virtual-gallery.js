(() => {
  ...
  function animateLayoutChange(mutate){
    ...
    /* 把锚点按回原来的屏幕位置，并在整个版面动画期间逐帧维持——上面的卡片是渐渐收矮的，
       只在切换那一帧补一次不够。
       但页面已经到底（或到顶）时这个修正根本落不下去：舒适切紧凑会把文档压掉几千像素，
       滚在底部的那次切换里锚点位置已经够不到了。那时候再逐帧试就成了和浏览器的夹取互搏，
       屏幕上表现为来回抽搐（实测逐帧 top 在 -3 到 -195 之间跳）。所以试不动就放手，
       剩下的交给浏览器自己稳定——它会停在新的底部，这本来就是这种情况下最合理的结果。 */
    let anchorMisses=0;
    const keepAnchor=()=>{
    ...
    for(const [uid,slot] of slots){
      ...
      if(slot.getBoundingClientRect().bottom<0)continue;
