(() => {
  let observer,resize;const mounted=new Map(),heights=new Map();
  ...
  function render(container,rows,makeCard){
    ...
    observer=new IntersectionObserver(entries=>{for(const entry of entries){const slot=entry.target,id=slot.dataset.uid;
      ...
      else if(!entry.isIntersecting&&mounted.has(id)){const h=slot.getBoundingClientRect().height;heights.set(id,h);resize.unobserve(slot);ArtistImages.dispose('card:'+id);slot.replaceChildren();slot.style.height=h+'px';mounted.delete(id);}
    }},{rootMargin:'1000px'});
  ...
  window.ArtistGallery={render,clear,visible(){return [...mounted.values()];}};
})();
