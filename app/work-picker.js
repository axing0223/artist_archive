(function(root){
  ...
  function mount(container,{uid,tag,exclude,preset,onPreview=()=>{},order='id_desc',orderOptions=[],onAdd=null,onRemove=null,onOrderChanged,onAlign=null}={}){
    ...
    /* 翻页不动焦点、也不动滚动位置：用户正在看候选列表，翻一页就跳一次视线很烦。
       焦点留在原地，键盘的 A / D 与左右方向键照旧能用（判据是"焦点还在这个容器里"，
       而翻页本来就没把焦点移走）。 */
    const previous=btn('← 上一页',()=>go(page-1)),next=btn('下一页 →',()=>go(page+1)),pageLabel=el('span','picker-page','第 1 页');
    previous.title='上一页：A / ←';next.title='下一页：D / →';pageLabel.setAttribute('aria-live','polite');
    pages.append(previous,pageLabel,next);
    const retry=btn('重试',()=>go(failedPage,false,failedOrder));retry.hidden=true;
    ...
    container.addEventListener?.('keydown',event=>{
      ...
      const delta=key==='a'||key==='arrowleft'?-1:1;if((delta<0&&previous.disabled)||(delta>0&&next.disabled))return;go(page+delta);
    });
