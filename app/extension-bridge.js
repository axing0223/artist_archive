(() => {
  ...
  window.ArtistExtension={get connected(){return connected;},async check(){try{const result=await request('ping');connected=true;return result.version;}catch(error){connected=false;throw error;}},async image(url,signal){if(!connected)await this.check();const r=await request('image',url,signal);if(!/^data:image\/(jpeg|png|webp|gif|avif);base64,[a-zA-Z0-9+/=]+$/.test(r.data))throw Error('扩展返回了无效图片');return (await fetch(r.data)).blob();}};
})();
